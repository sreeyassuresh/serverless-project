...
const certURL = 'https://dev-bp45zx6s.us.auth0.com/.well-known/jwks.json'
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  try {
      const certificate = await Axios.get(certURL);
      const signing = certificate.data.keys.filter(k => k.kid === kid)[0];
